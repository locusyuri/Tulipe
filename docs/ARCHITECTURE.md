...
### 1. 前端 ↔ Rust 代理 ↔ SpringBoot 通信

#### 通信方式

| 方式             | 协议                | 场景           | 特点                     |
| ---------------- | ------------------- | -------------- | ------------------------ |
| Tauri IPC → HTTP | JSON-RPC → HTTP/1.1 | 普通业务请求   | 安全、统一入口、易于管理 |
| Tauri IPC → HTTP | JSON-RPC → HTTP/1.1 | 服务端推送通知 | 安全、统一处理           |
| Tauri IPC → HTTP | JSON-RPC → HTTP/1.1 | 实时双向通信   | 安全、统一处理           |

#### 主要职责

```
┌─────────────────────────────────────────────────────────────────┐
│                前端 → Rust 代理 → SpringBoot                    │
├─────────────────────────────────────────────────────────────────┤
│ ✅ 数据库连接管理                                                │
│    - 创建/编辑/删除数据源连接                                     │
│    - 连接测试、连接池管理                                         │
...
export async function requestBackendJson<T>(
    ...
    const response = await invoke<BackendProxyResponse>("proxy_backend_request", { request });
    if (!response.ok) {
...
│  │ • 状态管理 (Pinia)                                                  │   │
│  │ • 路由控制                                                          │   │
│  │ • 调用 Rust (Tauri IPC) - 不直接调用 SpringBoot                    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │ • 全局快捷键                                                        │   │
│  │ • 窗口控制                                                          │   │
│  │ • 后端代理 (转发请求到 SpringBoot)                                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ SpringBoot 层 (GraalVM Native)                                     │   │
│  │ ─────────────────────────────────────────────────────────────────── │   │
│  │ • 数据库连接管理 (JDBC)                                             │   │
│  │ • SQL 执行与结果处理                                               │   │
│  │ • 业务逻辑处理                                                      │   │
...
| 功能类型           | 通信路径                      | 通信方式              | 原因                    |
| ------------------ | ----------------------------- | --------------------- | ----------------------- |
| 数据库连接管理     | 前端 → Rust 代理 → SpringBoot | Tauri IPC → HTTP      | 统一入口，安全管理      |
| SQL 执行（小数据） | 前端 → Rust 代理 → SpringBoot | Tauri IPC → HTTP      | 统一入口，安全管理      |
| SQL 执行（大数据） | 前端 → Rust → SpringBoot      | Tauri IPC → gRPC 流式 | Rust 处理流式传输更高效 |
...
| 用户配置           | 前端 → Rust 代理 → SpringBoot | Tauri IPC → HTTP      | 统一入口，安全管理      |
| AI 功能            | 前端 → Rust 代理 → SpringBoot | Tauri IPC → HTTP      | 统一入口，安全管理      |
...
import { requestBackendPostJson, requestBackendGetJson } from "./backendProxy";

export const api = {
	// 数据库相关 - 通过 Rust 代理调用 SpringBoot
	database: {
		async connect(config: ConnectionConfig) {
			return requestBackendPostJson<ConnectionConfig, Result<ConnectionInfo>>(
		...
		async executeQuery(sql: string, connectionId: string) {
			return requestBackendPostJson<{ sql: string; connectionId: string }, Result<QueryResult>>(
				"/api/database/query",
				{ sql, connectionId }
			);
		},
		...
		async getTables(connectionId: string) {
			return requestBackendGetJson<Result<List<TableInfo>>>(
				`/api/database/metadata/${connectionId}/tables`
			);
		},
	...
	async executeQuery(
		...
		if (estimatedSize > 100_000) {
			...
			// 小数据：通过 Rust 代理调用 SpringBoot
			return this.database.executeQuery(sql, connectionId);
		}
	},
	...
	async saveConnection(config: ConnectionConfig) {
		// 保存连接配置：通过 Rust 代理调用 SpringBoot
		const result = await this.database.saveConnection(config);
