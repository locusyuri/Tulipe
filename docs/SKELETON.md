...
- **优势**:
  ...
  - CodeMirror6 提供高性能、可定制的 SQL 编辑器
  - AG Grid 提供高性能、可定制的表格组件
...
**通信方式**: Tauri IPC → HTTP REST / WebSocket / SSE
...
```typescript
// 前端通过 Rust 代理调用 SpringBoot API
import { requestBackendPostJson } from "@/api/backendProxy";

const result = await requestBackendPostJson<{ sql: string; connectionId: string }, any>(
	"/api/database/query",
	{ sql, connectionId }
);
```
...
│  │                                                             ││
│  │  • 通过 Tauri IPC 访问配置数据                              ││
│  │  • 通过 Tauri IPC 调用 Rust 代理访问 SpringBoot 业务逻辑     ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│           ↓ HTTP                                                │
│           │                                                     │
...
Rust 启动嵌入式 src-backend.exe (子进程)
       ↓
前端通过 Tauri IPC 调用 Rust 代理
       ↓
Rust 代理转发请求到 SpringBoot 后端
       ↓
应用正常运行
       ↓
用户关闭应用 → Rust 终止后端子进程
