...
| 层级        | 技术栈                              | 核心职责                                           | 通信方式                                     |
| ----------- | ----------------------------------- | -------------------------------------------------- | -------------------------------------------- |
| **前端层**  | Vue3 + TypeScript + Tauri WebView   | UI 渲染、状态管理、业务逻辑编排                    | Tauri IPC → Rust（不直接与 SpringBoot 通信） |
| **Rust 层** | Rust 1.75+ + Tauri                  | 进程管理、文件系统、安全凭证、大数据处理、后端代理 | Tauri IPC ← 前端<br>HTTP/gRPC → SpringBoot   |
| **后端层**  | SpringBoot 3.2.4 + Kotlin + GraalVM | 数据库连接、JDBC 驱动、AI 服务、业务逻辑           | HTTP/gRPC ← Rust（不直接与前端通信）         |

### 通信路径决策

| 功能类型           | 通信路径                      | 原因                    |
| ------------------ | ----------------------------- | ----------------------- |
| 数据库连接管理     | 前端 → Rust 代理 → SpringBoot | 统一入口，安全管理      |
| SQL 执行（小数据） | 前端 → Rust 代理 → SpringBoot | 统一入口，安全管理      |
